...
function Frames({ images, q = new THREE.Quaternion(), p = new THREE.Vector3() }) {
	...
	useFrame((state, dt) => {
		easing.damp3(state.camera.position, p, 0.4, dt); // this makes the camera move to the clicked frame
		easing.dampQ(state.camera.quaternion, q, 0.4, dt); // this makes the camera turn to the clicked frame
		// damp3 takes 4 arguments: the first is the object to be damped, the second is the target, the third is the damping factor, and the fourth is the time delta
	});
	return (
		<group
			ref={ref}
