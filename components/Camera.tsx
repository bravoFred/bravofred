...
export default function Camera() {
	...
	useFrame((state) => {
		...
		state.camera.position.x = MathUtils.lerp(state.camera.position.x, state.pointer.x * 5, 0.2);
		state.camera.position.y = MathUtils.lerp(state.camera.position.y, state.pointer.y * 2, 0.1);
	});
